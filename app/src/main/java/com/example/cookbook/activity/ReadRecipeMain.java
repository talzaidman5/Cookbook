...
public class ReadRecipeMain extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_read_recipe_main);
    ...
    private void editRecipe(){
        ...
        listView_ingredient.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, final int position, long id) {
                AlertDialog alertDialog = new AlertDialog.Builder(ReadRecipeMain.this).create();
                alertDialog.setTitle("עריכה");

                final EditText input = new EditText(ReadRecipeMain.this);
        ...
        popup_BTN_yes.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                account.updateRecipesMain(recipe,recipeIndex);
     //           myRef.child("Users").child(account.getUserPhoneNumber()).child("Main").setValue(account.getRecipesMain());
    ...
    private int checkNullName(String name){
        if(name.length() ==0)
            return 1;

            for (int i =0; i< account.getRecipesMain().size();i++)
            {
                if(account.getRecipesMain().get(i).getName().equals(name))
                    return 2;
            }
        ...
        alertDialog.setButton(AlertDialog.BUTTON_NEUTRAL, "OK",
                new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        //        account.setRecipesMain(allRecipesMain);
               //         myRef.child("Users").child(account.getUserPhoneNumber()).child("Main").setValue( account.getRecipesMain());
                        myRef.child("Users").child(account.getUserPhoneNumber()).setValue( account);
                        putOnMSP();
                        // getFromMSP();
